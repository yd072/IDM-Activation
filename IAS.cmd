...
reg add "HKEY_CURRENT_USER\Console" /v "FontSize" /t REG_DWORD /d 0x00120000 /f >nul
reg add "HKEY_CURRENT_USER\Console" /v "FontWeight" /t REG_DWORD /d 400 /f >nul

:: 设置标题
title IDM 激活工具 / IDM Activation Tool %iasver%
...
echo _______________________________________________
echo:
echo [1] 激活 IDM          [1] Activate IDM
echo [2] 冻结试用期        [2] Freeze Trial  
echo [3] 重置激活/试用     [3] Reset Activation/Trial
echo _______________________________________________
echo:
echo [4] 下载 IDM          [4] Download IDM
echo [5] 帮助              [5] Help
echo [0] 退出              [0] Exit
echo _______________________________________________
echo:

choice /C:123450 /N /M "请选择操作/Please choose [1,2,3,4,5,0]: "
set _erl=%errorlevel%

if %_erl%==6 exit /b
if %_erl%==5 start https://www.internetdownloadmanager.com/download.html & goto MainMenu
if %_erl%==4 goto help_menu
...
echo %line%
echo:
echo      部分用户激活可能无效,IDM 可能会显示假序列号提示。
echo:
...
call :_color %Green% "IDM 30天试用期已成功永久冻结。"
echo:
call :_color %Gray% "如果 IDM 显示注册提示,请重新安装 IDM。"
)
...
:log_action
echo [%date% %time%] %* >> "%SystemRoot%\Temp\idm_activation.log"
exit /b

:check_update
set "_current_ver=%iasver%"
set "_update_url=https://api.example.com/version"
%psc% "(New-Object Net.WebClient).DownloadString('%_update_url%')" > "%temp%\ver.txt"
set /p _latest_ver=<"%temp%\ver.txt"
if "%_latest_ver%" GTR "%_current_ver%" (
    call :_color %Yellow% "发现新版本 %_latest_ver%"
    choice /C YN /M "是否更新?"
    if !errorlevel!==1 start "" "https://example.com/download"
)
...
:check_installation
if not exist "%IDMan%" (
    call :_color %Red% "IDM 未安装"
    exit /b 1
)
call :_color %Green% "IDM 安装正常"
exit /b 0
...
if "%LANG%"=="zh_CN" (
    ...
    set "MSG_ACTIVATE=Activate"
    set "MSG_BACKUP=Backup"
    set "MSG_RESTORE=Restore"
)
